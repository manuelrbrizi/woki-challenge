...
import { DiscoverSeatsQuery, DiscoverSeatsResponse } from '../dto/discover-seats.dto';
import {
  ListBookingsQuery,
...
export class BookingQueryService {
  ...
  async discoverSeats(query: DiscoverSeatsQuery): Promise<DiscoverSeatsResponse> {
  ...
  async findCandidates(
    ...
    bookings: Array<{ tableIds: string[]; start: Date; end: Date; status: string }>,
    date: Date,
    ...
    const comboCandidates = await this.findComboCandidates(
      tables,
      bookings,
      date,
      durationMinutes,
  ...
  private async findComboCandidates(
    tables: Array<{ id: string; minSize: number; maxSize: number }>,
    bookings: Array<{ tableIds: string[]; start: Date; end: Date; status: string }>,
    date: Date,
    durationMinutes: number,
    ...
    for (const combo of combinations) {
      const { minCapacity, maxCapacity } = this.comboCalculatorService.calculateCapacity(
        combo as any,
      );
